...
The reference file also flags fixes the migration must make to the existing `_publications/` folder: a wrong SSRN ID on the Visual Saliency paper, a renamed paper (Controlling Shareholders → Voting Rules and the Price of Peace), three missing working papers (Singapore Equity Market Reform, Neobrokers, Fork in Boardroom), and three SSRN-vs-DOCX title mismatches. **Per user direction (2026-05-26), the SSRN title is authoritative** wherever it differs from the DOCX. Post-migration the publications collection should have **19 entries** total.
